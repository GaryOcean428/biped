...
class RouteValidator:
    ...
    def validate_field_lengths(self, data: Dict[str, Any], field_lengths: Dict[str, int]) -> None:
        ...
        for field, max_length in field_lengths.items():
            if field in data and isinstance(data[field], str) and len(data[field]) > max_length:
                length_errors.append(f"{field} must be no more than {max_length} characters")

        if length_errors:
            raise ValidationError(f"Length validation errors: {'; '.join(length_errors)}")

    def validate_numeric_ranges(self, data: Dict[str, Any], numeric_ranges: Dict[str, Dict[str, Union[int, float]]]) -> None:
    ...
    def validate_enum_values(self, data: Dict[str, Any], enum_fields: Dict[str, List[str]]) -> None:
        """Validate enum/choice fields"""
        enum_errors = []

        for field, allowed_values in enum_fields.items():
            if field in data and data[field] not in allowed_values:
                enum_errors.append(f"{field} must be one of: {', '.join(allowed_values)}")
    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            if 'required_fields' in validation_config:
                validator.validate_required_fields(data, validation_config['required_fields'])
            ...
            if 'field_lengths' in validation_config:
                validator.validate_field_lengths(data, validation_config['field_lengths'])
            
            if 'numeric_ranges' in validation_config:
                validator.validate_numeric_ranges(data, validation_config['numeric_ranges'])
            ...
            for key, value in data.items():
                if isinstance(value, str):
                    max_length = validation_config.get('field_lengths', {}).get(key, 255)
    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            if 'required_params' in validation_config:
                validator.validate_required_fields(params, validation_config['required_params'])
                ...
                for param, expected_type in validation_config['param_types'].items():
                    if param in params:
                        try:
                            ...
                            raise ValidationError(f"Parameter {param} must be of type {expected_type.__name__}")
...
def validate_job_creation():
    ...
    return validate_json_request({
        'required_fields': ['title', 'description', 'category', 'budget_min', 'budget_max'],
        'field_types': {
            'title': str,
            'description': str,
            'category': str,
