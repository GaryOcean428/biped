    ...
    def get_cv_status() -> Dict[str, Any]:
        ...
        try:
            ...
            status["libraries"]["opencv"] = {
                ...
                "build_info": cv2.getBuildInformation()[:200] + "..." if len(cv2.getBuildInformation()) > 200 else cv2.getBuildInformation()
            }
        except Exception as e:
            status["libraries"]["opencv"] = {
                "available": False,
        ...
        status["available"] = (
            status["libraries"].get("opencv", {}).get("available", False) and
            status["libraries"].get("pillow", {}).get("available", False) and
            status["libraries"].get("numpy", {}).get("available", False)
        )
