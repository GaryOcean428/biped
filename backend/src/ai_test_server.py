...
def create_ai_test_app():
    ...
    def health_check():
        ...
        return jsonify({
            ...
            'ai_providers': {provider.value: available for provider, available in enhanced_ai.router.available_providers.items()} if enhanced_ai else {}
        })
    ...
    def find_matches():
        ...
        try:
            ...
            required_fields = ['title', 'description', 'category', 'budget_min', 'budget_max', 'location', 'urgency']
            ...
            if enhanced_ai:
                ...
                MatchResult = make_dataclass('MatchResult', [
                    'provider_id', 'match_score', 'skill_match', 'location_score',
                    'budget_compatibility', 'availability_score', 'quality_score', 'explanation'
                ])
                matches = [MatchResult(**{k: v for k, v in match.items() if k in [
                    'provider_id', 'match_score', 'skill_match', 'location_score',
                    'budget_compatibility', 'availability_score', 'quality_score', 'explanation'
                ]}) for match in matches]
            else:
                matches = traditional_ai.find_matches(job, mock_providers, top_k=5) if traditional_ai else []
    ...
    def predict_demand():
        ...
        try:
            ...
            prediction = enhanced_ai.predict_demand_with_ai(category, location, days_ahead)
