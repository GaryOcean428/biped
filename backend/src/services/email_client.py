...
class EmailMicroserviceClient:
    ...
    def send_template_email(
        self, to_emails: List[str], template_id: str, template_data: Dict[str, Any]
    ) -> bool:
...
class EnhancedNotificationService:
    ...
    def send_quote_notification(
        ...
        if self.use_microservice:
            ...
            html_content = self._get_quote_notification_html(
                customer_name, provider_name, job_title, quote_amount
            )

            return self.email_client.send_email([customer_email], subject, html_content)

    def send_payment_confirmation_email(
        self, user_email: str, user_name: str, amount: float, job_title: str
    ) -> bool:
