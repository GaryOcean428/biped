...
class UserService:
        ...
        try:
            ...
            if user_type == 'customer':
                profile = CustomerProfile(
                    user_id=user.id,
                    preferred_contact_method=user_data.get('preferred_contact_method', 'email'),
    ...
    @staticmethod
    def update_user_profile(user_id: int, profile_data: Dict, user_type: str = 'user') -> Dict:
        ...
        try:
            if user_type == 'admin':
                ...
                if user.user_type == 'customer':
                    profile = CustomerProfile.query.filter_by(user_id=user.id).first()
                    if profile:
                        profile_fields = ['preferred_contact_method', 'notification_preferences']
                    ...
                    if profile:
                        profile_fields = [
                            'business_name', 'abn', 'years_experience', 'hourly_rate',
                            'bio', 'skills', 'availability'
                        ]
                        for field in profile_fields:
                            if field in profile_data:
                                setattr(profile, field, profile_data[field])
    ...
    @staticmethod
    def change_password(user_id: int, current_password: str, new_password: str, user_type: str = 'user') -> bool:
    ...
    def get_user_dashboard_data(user_id: int, user_type: str = 'user') -> Dict:
        ...
        try:
            ...
            current_app.logger.error(f"Error getting dashboard data for user {user_id}: {e}")
...
class RoleBasedAccessControl:
    ...
    @staticmethod
    def can_access_resource(user_type: str, resource: str, action: str = 'view') -> bool:
