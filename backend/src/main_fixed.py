...
print(
    f"🗄️ Database: {'PostgreSQL (Railway)' if app.config['SQLALCHEMY_DATABASE_URI'].startswith('postgresql') else 'SQLite (Local)'}"
)
...
def root():
    ...
    main_app_files = ["dashboard-enhanced.html", "index.html", "dashboard.html"]
...
def dashboard():
    ...
    dashboard_files = ["dashboard-enhanced.html", "dashboard.html"]
...
def jobs():
    ...
    job_files = ["enhanced-job-posting.html", "jobs.html"]
...
def providers():
    ...
    provider_files = ["provider-dashboard.html", "providers.html"]
