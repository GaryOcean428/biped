...
def update_settings():
    ...
    try:
        ...
        updated_user = UserService.update_user_profile(user_id, settings_data, user_type)
...
def update_notifications():
    ...
    try:
        ...
        updated_user = UserService.update_user_profile(user_id, notification_data, user_type)
...
def check_permission():
    ...
    try:
        ...
        has_permission = RoleBasedAccessControl.has_permission(user_type, data['permission'])
            ...
            for user_id in user_ids:
                try:
                    ...
                    if not result['user']['is_active']:
                        ...
                        UserService.toggle_user_status(user_id, admin_id)
                        results.append({'user_id': user_id, 'status': 'already_inactive'})
                except Exception as e:
                    results.append({'user_id': user_id, 'status': 'error', 'error': str(e)})
        
        else:
            return jsonify({'error': 'Invalid action. Supported actions: activate, deactivate'}), 400
