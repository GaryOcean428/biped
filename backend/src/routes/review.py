...
def get_user_reviews(user_id):
    ...
    try:
        ...
        query = Review.query.filter_by(reviewee_id=user_id, is_public=True).order_by(Review.created_at.desc())
        ...
        if all_reviews:
            stats['average_rating'] = sum([r.overall_rating for r in all_reviews]) / len(all_reviews)
...
def get_pending_reviews():
    ...
    try:
        ...
        for job in completed_jobs:
            ...
            reviewee_id = job.assigned_provider_id if job.customer_id == user_id else job.customer_id
