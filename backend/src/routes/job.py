...
def create_job():
    ...
    try:
        ...
        required_fields = ['service_id', 'title', 'description', 'street_address', 'city', 'state', 'postcode']
        ...
        job = Job(
            ...
            preferred_start_date=datetime.strptime(data['preferred_start_date'], '%Y-%m-%d').date() if data.get('preferred_start_date') else None,
...
def get_job(job_id):
    ...
    try:
        ...
        elif user.user_type == 'provider' and job.assigned_provider_id != user_id and job.status not in [JobStatus.POSTED, JobStatus.MATCHED]:
            return jsonify({'error': 'Access denied'}), 403
        
        # Get quotes if user is customer or assigned provider
        quotes = []
        if user.user_type == 'customer' or job.assigned_provider_id == user_id:
            quotes = Quote.query.filter_by(job_id=job_id, is_active=True).order_by(Quote.price).all()
        
        # Get messages
        messages = JobMessage.query.filter_by(job_id=job_id).order_by(JobMessage.created_at).all()
        
        # Get milestones
        milestones = JobMilestone.query.filter_by(job_id=job_id).order_by(JobMilestone.sort_order).all()
...
def create_quote():
    ...
    try:
        ...
        quote = Quote(
            ...
            estimated_start_date=datetime.strptime(data['estimated_start_date'], '%Y-%m-%d').date() if data.get('estimated_start_date') else None,
            estimated_completion_date=datetime.strptime(data['estimated_completion_date'], '%Y-%m-%d').date() if data.get('estimated_completion_date') else None,
            ...
            valid_until=datetime.strptime(data['valid_until'], '%Y-%m-%d %H:%M:%S') if data.get('valid_until') else None
        )
