...
def search_services():
    ...
    try:
        ...
        if min_rating > 0:
            provider_services_query = provider_services_query.filter(
                ProviderService.average_rating >= min_rating
            )
        ...
        provider_services_query = provider_services_query.order_by(
            ProviderService.average_rating.desc(), ProviderService.jobs_completed.desc()
        )
...
def get_provider_details(provider_id):
    ...
    try:
        ...
        # Get portfolio items
        portfolio = (
            PortfolioItem.query.filter_by(provider_id=provider_id, is_public=True)
            .order_by(PortfolioItem.is_featured.desc(), PortfolioItem.sort_order)
            .all()
        )
        ...
        reviews = (
            Review.query.filter_by(reviewee_id=user.id, is_public=True)
...
def add_provider_service():
    ...
    try:
        ...
        existing = ProviderService.query.filter_by(
            provider_id=provider.id, service_id=data["service_id"]
        ).first()
