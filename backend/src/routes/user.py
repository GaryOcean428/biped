        ...
        if user.user_type == 'customer':
            ...
            dashboard_data = {
                ...
                'active_jobs': Job.query.filter_by(
                    customer_id=user_id,
                    is_active=True
                ).filter(Job.status.in_([JobStatus.POSTED, JobStatus.MATCHED, JobStatus.ACCEPTED, JobStatus.IN_PROGRESS])).count(),
                'completed_jobs': Job.query.filter_by(
                    customer_id=user_id,
                    status=JobStatus.COMPLETED
                ).count(),
                'pending_reviews': Review.query.join(Job).filter(
                ...
                'recent_jobs': [job.to_dict() for job in Job.query.filter_by(
                    customer_id=user_id
            ...
            if provider:
                dashboard_data = {
                    ...
                    'active_jobs': Job.query.filter_by(
                        assigned_provider_id=user_id
                    ).filter(Job.status.in_([JobStatus.ACCEPTED, JobStatus.IN_PROGRESS])).count(),
        ...
        for user in pagination.items:
            ...
            if user.user_type == 'provider' and user.provider_profile:
                user_data.update({
                    ...
                    'average_rating': user.provider_profile.average_rating,
                    'total_jobs_completed': user.provider_profile.total_jobs_completed,
                    'hourly_rate': float(user.provider_profile.hourly_rate) if user.provider_profile.hourly_rate else None
                })
