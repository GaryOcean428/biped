...
class ResponseTimeTests(PerformanceTestCase):
    ...
    def test_health_check_response_time(self):
        ...
        self.assertLess(response_time, 100, f"Health check took {response_time:.2f}ms, should be under 100ms")
    ...
    def test_detailed_health_check_response_time(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertLess(response_time, 500, f"Detailed health check took {response_time:.2f}ms, should be under 500ms")
    ...
    def test_authentication_endpoint_response_time(self):
        ...
        self.assertLess(response_time, 200, f"Authentication took {response_time:.2f}ms, should be under 200ms")
    ...
    def test_services_listing_response_time(self):
        ...
        self.assertLess(response_time, 300, f"Services listing took {response_time:.2f}ms, should be under 300ms")
...
class ConcurrencyTests(PerformanceTestCase):
    ...
    def test_concurrent_health_checks(self):
        ...
        self.assertLess(avg_response_time, 150, f"Average response time {avg_response_time:.2f}ms should be under 150ms")
        self.assertLess(max_response_time, 300, f"Max response time {max_response_time:.2f}ms should be under 300ms")
    ...
    def test_concurrent_authentication_attempts(self):
        ...
        self.assertEqual(successful_responses, num_requests, 
        ...
        self.assertEqual(len(server_errors), 0, f"No server errors should occur, got: {server_errors}")
...
class CachePerformanceTests(PerformanceTestCase):
    ...
    def test_response_caching_effectiveness(self):
        ...
        self.assertLess(second_response_time, 500, "Second request should be under 500ms")
        
        # Log the performance difference
        print(f"First request: {first_response_time:.2f}ms, Second request: {second_response_time:.2f}ms")
...
class CompressionTests(PerformanceTestCase):
    ...
    def test_compression_content_integrity(self):
        ...
        response_with_compression = self.client.get('/api/health/detailed',
                                                  headers={'Accept-Encoding': 'gzip'})
        ...
        try:
            ...
            if 'Content-Encoding' not in response_with_compression.headers:
                self.assertEqual(response_no_compression.data, response_with_compression.data)
...
class DatabasePerformanceTests(PerformanceTestCase):
    ...
    def test_database_connection_performance(self):
        ...
        with self.app.app_context():
            ...
            self.assertLess(query_time, 50, f"Database query took {query_time:.2f}ms, should be under 50ms")
    ...
    def test_multiple_database_queries_performance(self):
        ...
        with self.app.app_context():
            ...
            self.assertLess(avg_time, 10, f"Average query time {avg_time:.2f}ms should be under 10ms")
            self.assertLess(total_time, 100, f"Total time {total_time:.2f}ms should be under 100ms")
